...
function Process-Static($inPath, $outPath, $size) {
...
function Process-SpriteSheet($inPath, $outPath, $frames, $size) {
    $img = [System.Drawing.Image]::FromFile($inPath)
    $bmp = New-Object System.Drawing.Bitmap($img)
    $img.Dispose()
    ...
    $frameH = $bmp.Height
    $sheetW = $size * $frames
    $sheet = New-Object System.Drawing.Bitmap($sheetW, $size)
    ...
    $sheet.Dispose()
    Write-Host "OK sheet: $outPath"
}

function Process-Tile($inPath, $outPath, $size) {
    $img = [System.Drawing.Image]::FromFile($inPath)
    $result = New-Object System.Drawing.Bitmap($size, $size)
    $g = [System.Drawing.Graphics]::FromImage($result)
    $g.InterpolationMode = [System.Drawing.Drawing2D.InterpolationMode]::NearestNeighbor
